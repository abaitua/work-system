...
async function getTimeConfig() {
  ...
  try {
    const res = await redis.get(TIME_CONFIG_KEY);
    if (!res) {
      await redis.set(TIME_CONFIG_KEY, DEFAULT_TIME_CONFIG);
      return DEFAULT_TIME_CONFIG;
    }
    return res;
  } catch {
    return memoryTimeConfig;
  }
}
async function setTimeConfig(config) {
  memoryTimeConfig = JSON.parse(JSON.stringify(config));
  if (!redisAvailable) return;
  try { await redis.set(TIME_CONFIG_KEY, config); } catch {}
}
